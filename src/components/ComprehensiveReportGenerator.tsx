...
import type { ReportTemplate, GeneratedReport, ReportData, BehaviorEntry, MedicationLog, Goal, Appointment, CrisisPlan } from '../types/components';
...
// report_templates.sections and generated_reports.report_data are stored as
// Json; these guards rebuild the typed shapes this component works with
// from whatever comes back out of the database.
...
const asString = (value: Json | undefined, fallback = ''): string =>
  typeof value === 'string' ? value : fallback;

const asNullableString = (value: Json | undefined): string | null =>
  typeof value === 'string' ? value : null;

const asNumber = (value: Json | undefined, fallback = 0): number =>
  typeof value === 'number' ? value : fallback;

const asNullableNumber = (value: Json | undefined): number | null =>
  typeof value === 'number' ? value : null;

const asNullableBoolean = (value: Json | undefined): boolean | null =>
  typeof value === 'boolean' ? value : null;

const asNullableStringArray = (value: Json | undefined): string[] | null =>
...
const parseReportTemplateSections = (sections: Json): ReportTemplate['sections'] =>
  asArray(sections).filter(isJsonRecord).filter(
    (s): s is { id: string; name: string; include: boolean } =>
      typeof s.id === 'string' && typeof s.name === 'string' && typeof s.include === 'boolean'
  );

const toReportTemplate = (row: Tables<'report_templates'>): ReportTemplate => ({
  ...row,
  sections: parseReportTemplateSections(row.sections ?? [])
});

const parseBehaviorEntry = (value: Json): BehaviorEntry | null => {
  if (!isJsonRecord(value)) return null;
  return {
    id: asString(value.id),
    user_id: asString(value.user_id),
    child_name: asString(value.child_name),
    behavior_type: asString(value.behavior_type),
    entry_date: asString(value.entry_date),
    entry_time: asString(value.entry_time),
    severity: asNumber(value.severity),
    duration_minutes: asNullableNumber(value.duration_minutes),
    location: asNullableString(value.location),
    antecedents: asNullableString(value.antecedents),
    consequences: asNullableString(value.consequences),
    triggers: asNullableStringArray(value.triggers),
    interventions_used: asNullableStringArray(value.interventions_used),
    effectiveness: asNullableNumber(value.effectiveness),
    notes: asNullableString(value.notes),
    created_at: asNullableString(value.created_at)
  };
};

const parseMedicationLog = (value: Json): MedicationLog | null => {
  if (!isJsonRecord(value)) return null;
  const medications = isJsonRecord(value.medications)
    ? { name: asString(value.medications.name), dosage: asString(value.medications.dosage) }
    : null;
  return {
    id: asString(value.id),
    user_id: asString(value.user_id),
    medication_id: asString(value.medication_id),
    taken_at: asString(value.taken_at),
    status: asString(value.status),
    scheduled_time: asNullableString(value.scheduled_time),
    logged_at: asNullableString(value.logged_at),
    behavioral_changes: asNullableString(value.behavioral_changes),
    side_effects_observed: asNullableString(value.side_effects_observed),
    notes: asNullableString(value.notes),
    medications
  };
};

const parseGoal = (value: Json): Goal | null => {
  if (!isJsonRecord(value)) return null;
  return {
    id: asString(value.id),
    user_id: asString(value.user_id),
    ...
    status: asString(value.status),
    target_value: asNumber(value.target_value),
    current_value: asNumber(value.current_value),
    unit: asString(value.unit),
    target_date: asNullableString(value.target_date),
    completed_at: asNullableString(value.completed_at),
    linked_condition: asNullableString(value.linked_condition),
    notes: asNullableString(value.notes),
    created_at: asNullableString(value.created_at)
  };
};

const parseAppointmentRow = (value: Json): Appointment | null => {
  if (!isJsonRecord(value)) return null;
  return {
    id: asString(value.id),
    user_id: asString(value.user_id),
    child_name: asString(value.child_name),
    appointment_type_id: asNullableString(value.appointment_type_id),
    appointment_date: asString(value.appointment_date),
    provider_name: asNullableString(value.provider_name),
    location: asNullableString(value.location),
    notes: asNullableString(value.notes),
...
const parseCrisisPlan = (value: Json): CrisisPlan | null => {
  if (!isJsonRecord(value)) return null;
  return {
    id: asString(value.id),
    user_id: asString(value.user_id),
    child_name: asString(value.child_name),
    warning_signs: asNullableStringArray(value.warning_signs),
    immediate_actions: asNullableStringArray(value.immediate_actions),
    things_to_avoid: asNullableStringArray(value.things_to_avoid),
    when_to_call_911: asNullableStringArray(value.when_to_call_911),
    safe_space_location: asNullableString(value.safe_space_location),
    medication_instructions: asNullableString(value.medication_instructions),
    additional_notes: asNullableString(value.additional_notes),
    created_at: asNullableString(value.created_at),
    updated_at: asNullableString(value.updated_at)
  };
};

const parseReportData = (json: Json): ReportData => {
  if (!isJsonRecord(json)) {
    return { generatedDate: '', dateRange: { start: '', end: '' } };
  }

  const dateRange = isJsonRecord(json.dateRange)
    ? { start: asString(json.dateRange.start), end: asString(json.dateRange.end) }
    : { start: '', end: '' };

  const data: ReportData = {
    generatedDate: asString(json.generatedDate),
    dateRange
  };

  if (isJsonRecord(json.behaviors)) {
    data.behaviors = {
      total: asNumber(json.behaviors.total),
      entries: asArray(json.behaviors.entries).map(parseBehaviorEntry).filter((e): e is BehaviorEntry => e !== null),
      summary: asString(json.behaviors.summary)
    };
  }

  if (isJsonRecord(json.medications)) {
    data.medications = {
      logs: asArray(json.medications.logs).map(parseMedicationLog).filter((l): l is MedicationLog => l !== null),
      adherenceRate: asNumber(json.medications.adherenceRate),
      summary: asString(json.medications.summary)
    };
  }

  if (isJsonRecord(json.goals)) {
    data.goals = {
      total: asNumber(json.goals.total),
      active: asNumber(json.goals.active),
      completed: asNumber(json.goals.completed),
      details: asArray(json.goals.details).map(parseGoal).filter((g): g is Goal => g !== null)
    };
  }

  if (isJsonRecord(json.appointments)) {
    data.appointments = {
      total: asNumber(json.appointments.total),
      attended: asNumber(json.appointments.attended),
      details: asArray(json.appointments.details).map(parseAppointmentRow).filter((a): a is Appointment => a !== null)
    };
  }

  const crisisPlans = asArray(json.crisisPlans).map(parseCrisisPlan).filter((c): c is CrisisPlan => c !== null);
...
const toGeneratedReport = (row: Tables<'generated_reports'>): GeneratedReport => ({
  ...row,
  report_data: parseReportData(row.report_data ?? null)
});
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          // reportData is a plain serializable object built from typed DB rows above;
          // round-trip through JSON so its shape structurally matches Json (mirrors what jsonb storage does anyway).
